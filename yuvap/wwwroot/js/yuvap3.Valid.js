                  ...
                  // var strOptList = document.getElementById("selectTopRow").innerHTML;                        
                   var numAttrib = 1; 
                   var strAttrib = numAttrib.toString() ;
                  //var strHtml1 = "";
              // alert('Global var in ..Valid.js - ' + numAttrib); //+ window.event.srcElement);
         ...
         // function addRow(in_tbl_name);
                    ...
                    function addRow(in_tbl_name){
              //strAttrib = numAttrib.toString(); // int to str // Causing prob. ?
  
                    // alert(' line 1 - ' + strHtml1); //+ window.event.srcElement);
                    // alert(' line 2 - ' + strAttrib); //+ window.event.srcElement);
                    // alert(' line 3 - ' + strOptList); //+ window.event.srcElement);
                    
                    
                    // alert('1st line - strAttrib = '    + strAttrib ); //strHtml1); //+ window.event.srcElement);
            ...
            //  td1.setAttribute(id,strAttrib );
                         ...
                         //alert("prev val " + td1.outerHTML(name) ); // try inline
                         
             // strOptList = document.getElementById("selectTopRow").innerHTML; //it is now global decl in page after populating 'Select Element'?
                       
                         var strHtml1 = "<select " + "id="+strAttrib+  " value=\"\" name=\"PreferredGirls\"  style=\"width: 90px;\" >" +  strOptList  + "</select>" 
                        // var strHtml1 = "<select " +   " value=\"\" name=\"PreferredGirls\"  style=\"width: 90px;\" >" +  strOptList  + "</select>" 
                         ...
                         // prev select ele    
           // var prevSelect = getElementById(strAttrib);
                         
                        // alert("addRow: - prev val "  + td1.innerHTML ); // try inline
            // alert(' last line - Attrib = ' + strAttrib + ' name: ' + prevSelect.name ); //window.event.srcElement);                  
                        
 
                  numAttrib = numAttrib + 1;   
                   strAttrib = numAttrib.toString(); // int to str
...
(4) Data validation:
 - Not possible due to  get 'array of pref', because of possible deletion of records by user. Now get it done by controller on server.
